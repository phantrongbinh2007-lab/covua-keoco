...
io.on('connection', (socket) => {
    ...
    socket.on('reconnectUser', (data) => {
        ...
        player.id = socket.id; socket.join(rCode);
        if (room.status === 'waiting') {
            socket.emit('roomCreated', { roomCode: rCode, isHost: room.hostToken === data.token });
    ...
    socket.on('createRoom', (settings) => {
        ...
        rooms[roomCode] = {
            ...
            activePlayers: [], bracket: [], currentRoundMatches: [],
            level: settings.level, winScore: parseInt(settings.winScore) || 3, status: 'waiting'
        };
    ...
    socket.on('joinRoom', (data) => {
        ...
        if (room && room.status === 'waiting') {
            ...
            if (existingPlayer) { existingPlayer.id = socket.id; existingPlayer.name = data.playerName; } 
            else { room.players.push({ id: socket.id, token: data.token, name: data.playerName }); }
            socket.join(rCode);
            socket.emit('roomCreated', { roomCode: rCode, isHost: false });
            io.to(rCode).emit('updateWaitingRoom', room.players);
        } else { socket.emit('errorMsg', 'Mã phòng không tồn tại hoặc đã bắt đầu!'); }
    });

    socket.on('startTournament', (roomCode) => {
        const rCode = String(roomCode).trim();
        const room = rooms[rCode];
